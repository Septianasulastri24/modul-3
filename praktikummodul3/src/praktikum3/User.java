...
public class User {
    ...
    private String newPassw, searchUser;
    ...
    private final Scanner objInput = new Scanner(System.in);
    ...
    protected void dashboard() {
        ...
        if (this.getUserDB()[this.getUserIndex()][2] == "0") {
            this.panelAdmin();
        } else {
            this.panelMhs();
        }
        ...
        if (pil == 1 && !this.getAdmin()) {
            do {
                ...
                newPassw = objInput.next();
                validation objVal = new validation(newPassw);
                if (!objVal.getFreturn()) {
                    System.out.println("Password Harus Kombinasi Huruf,Angka,Symbol Dan Uppercase");
                    check = true;
                } else {
                    this.setUserDB(this.getUserIndex(), 1, newPassw);
        ...
        if (pil == 1 && this.getAdmin()) {
            ...
            this.searchUser = objInput.next();
            for (int i = 0; i < this.getUserDB().length; i++) {
                if (searchUser.equalsIgnoreCase(this.getUserDB()[i][0])) {
